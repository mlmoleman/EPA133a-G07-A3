...
def create_links(df, indexes):
    ...
    for i in index[1:]:
        ...
        if not (lat == lat_prev and lon == lon_prev):
            ...
            km = (df.iloc[i, df.columns.get_indexer(['km'])].values[0]+df.iloc[i-1,
                  df.columns.get_indexer(['km'])].values[0])/2
            # Calculate the latitude of the link by interpolating
            lat = (df.iloc[i, df.columns.get_indexer(['lat'])].values[0]+df.iloc[i-1,
                   df.columns.get_indexer(['lat'])].values[0])/2
            # Calculate the longitude of the link by interpolating
            lon = (df.iloc[i, df.columns.get_indexer(['lon'])].values[0]+df.iloc[i-1,
                   df.columns.get_indexer(['lon'])].values[0])/2
            # Add the link to the dictionary with the necessary attributes
            inserting_links[i] = {'road': ['N1'], 'km': [km], 'type': ['link'], 'name': ['link'],
                                  'length': [round(length[0])], 'condition': None, 'lat': [lat],
                                  'lon': [lon], 'model_type': ['link']}
